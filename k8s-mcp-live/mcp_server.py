    ...
    try:
        ...
        for pod in pods.items:
            ...
            pod_list.append({
                ...
                "status": status,
                "ready": f"{sum(1 for c in container_statuses if c['ready'])}/{len(container_statuses)}" if container_statuses else "0/0",
        ...
        status_order = {"Failed": 0, "Pending": 1, "Unknown": 2, "Running": 3, "Succeeded": 4}
...
async def get_resource_usage(namespace: str = "default") -> dict:
    ...
    try:
        ...
        return {
            ...
            "pods": pod_resources,
            "recommendations": [
                f"{pods_without_limits} pods don't have resource limits set" if pods_without_limits > 0 else "All pods have resource limits ✓"
            ]
        }
    except ApiException as e:
        return {"error": f"API error: {e.reason}"}
